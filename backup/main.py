...
def main():
    ...
    parser.add_argument("--run_type", default="train", choices=["train", "test", "curves"], help="Run type: train, test, or curves")
    ...
    tag = f"{backbone_type}_{'skip' if use_skip else 'noskip'}_{'att' if use_attention else 'noatt'}"
    ...
    if run_type == "train":
        ...
        run_phase1(
            ...
            model_name=tag, num_epochs=PHASE1_EPOCHS, patience=PHASE1_PATIENCE)
        ...
        run_phase2(
            ...
            train_loss_history, val_loss_history, use_attention=use_attention, use_skip=use_skip,
            model_name=tag, num_epochs=PHASE2_EPOCHS, patience=PHASE2_PATIENCE)

        with open(f'plots/{tag}_curves.csv', 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['epoch', 'train_loss', 'val_loss', 'phase'])
            for i, (train_loss, val_loss) in enumerate(zip(train_loss_history, val_loss_history)):
                phase = 1 if i < phase1_len else 2
                writer.writerow([i + 1, train_loss, val_loss, phase])
